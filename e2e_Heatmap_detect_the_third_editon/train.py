...
def train(model,train_detect_dataloader,val_detect_dataloader,train_pose_dataloader,val_pose_dataloader,cfg,device,writer_dict):
            ...
            with tqdm(total=epoch_steps_train,ncols=100,desc=f"train:{epoch+1}/{num_epochs}",postfix=dict) as pbar:
                for i , (images , boxes ) in enumerate(train_detect_dataloader):
                    ...
                    if i % cfg.PRINT_FREQ == 0:
                        ...
                        writer.add_scalar('train_detect_loss', loss.item(), train_detect_global_steps)
                        writer_dict['train_detect_global_steps'] = train_detect_global_steps + 1
                    
                    pbar.set_postfix(**{"averg_loss":(train_loss+loss.item())/(i+1)})
                    pbar.update(1)
        ...
        for epoch in range(num_epochs):
            ...
            with tqdm(total=epoch_steps_train,ncols=100,desc=f"train:{epoch+1}/{num_epochs}",postfix=dict) as pbar:
                for i , (images , gt_boxes , gt_heatmaps) in enumerate(train_pose_dataloader):
                    ...
                    loss = Pose_loss(pose_outs,heatmaps)
                    ...
                    pbar.set_postfix(**{"pose_loss":(train_loss+loss)/i+1})
                    pbar.update(1)
...
def main():
    ...
    train_detect_dataloader = DataLoader(dataset=train_detect_dataset,batch_size=cfg.batch_size,shuffle=True,num_workers=4,pin_memory=True,collate_fn=detect_dataset_collate)
    ...
    train_pose_dataloader = DataLoader(dataset=train_pose_dataset,batch_size=cfg.batch_size//2,shuffle=True,num_workers=4,pin_memory=True,collate_fn=pose_collate_fn)
...
def parse_args(**kwargs):
    ...
    parser.add_argument("--training_yolo", help="train_yolo only" , default=True)
    parser.add_argument("--training_pose", help="train_pose only" , default=False)
