...
class PoseNet(nn.Module):
    def __init__(self, nstack=2, in_dim=768, out_dim=17, bn=False, increase=0, **kwargs):
        ...
        self.merge_features = nn.ModuleList( [Merge(in_dim, in_dim) for i in range(nstack-1)] )
        self.merge_preds = nn.ModuleList( [Merge(out_dim, in_dim) for i in range(nstack-1)] )
        self.nstack = nstack
