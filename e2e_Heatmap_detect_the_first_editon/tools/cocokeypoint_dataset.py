...
class CocoKeypoint(CocoDataset):
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            annos = [anno for anno in annos if anno['iscrowd']==0 and anno["num_keypoints"]>0]
            
            if len(bboxes)==0:
                img = self.get_transforms(img)      # resize , totensor , # (3,image_size,image_size)
                fake_person_boxs = self.get_random_boxes(n=self.cfg.num_of_fake_person)
                fake_heatmaps = torch.zeros(self.cfg.num_of_fake_person,17,self.heatmap_size,self.heatmap_size)
                return img , fake_person_boxs , fake_heatmaps
            if len(bboxes)!=0 and len(annos)==0:
                img , annos  = self.select_another_img()
            else:
                break
        ...
        #   heatmaps.shape = (n_person,17,h,w)      # h,w = output_heatmap_size = 64
        """
    ...
    def adjust_box(self,annos,img_h,img_w,adjust=False):
        """
        将人体框限制在图片范围内
    ...
    def get_random_boxes(self,n=1):
        fake_boxes = []
        for i in range(n):
            x = random.randint(1,self.image_size-20)
            y = random.randint(1,self.image_size-30)
            w = random.randint(10,20)
            h = random.randint(15,25)
            fake_boxes.append([x,y,w,h])
        fake_boxes = torch.tensor(fake_boxes,dtype=torch.float32)

        return fake_boxes

    def select_another_img(self):
