...
class Detect_Dataset(CocoDataset):
    def __init__(self,root, anno_name, image_size, use_mosaic=True, is_train=False):
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            if self.use_mosaic and self.rand() > 0.5 :
                ...
                box: shape=(n,5)        # 5 = x1,y1,w,h,id
                new_box: shape=(n,5)    # 5 = cx,cy,w,h,id      # gui yi hua
                """
                img = self.totensor(img)
                new_box = box
                new_box[:,:4] = new_box[:,:4]/self.image_size
                new_box[:, 2:4] = new_box[:, 2:4] - new_box[:, 0:2]
                new_box[:, 0:2] = new_box[:, 0:2] + new_box[:, 2:4] / 2
                return img , new_box
