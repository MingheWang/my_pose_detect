...
class E2EModel(nn.Module):
    def __init__(self,cfg):
        ...
        if self.training_yolo or not self.training:
        ...
        if self.training_pose or not self.training:
            self.roi_heads = [RoIAlign(output_size=(16,16),spatial_scale=8,sampling_ratio=2),RoIAlign(output_size=(16,16),spatial_scale=16,sampling_ratio=2)] 
    ...
    def forward(self,x,gt_person_boxes=None ,gt_heatmaps=None):
        ...
        #------------------------------------------#
        device = x.device
        print("\nimage is in {} device".format(device))
        x2, x1, x0 = self.backbone(x)

        if self.training_yolo or not self.training:
            out2,out1,out0 = self.detect_net(x2,x1,x0)
            print("after_detect_net    =:{}".format(torch.cuda.memory_allocated(0)))
        ...
        if self.training:
            if self.training_yolo:
                ...
                img_with_person = torch.tensor([ torch.sum(boxes)>0 for boxes in gt_person_boxes])

                img_with_person_cpu = img_with_person.clone().cpu()
                mask = np.array(img_with_person_cpu).repeat(num_boxes_per_img,axis=0)
                mask = torch.from_numpy(mask).to(device)

                img_id = np.arange(len(gt_person_boxes))
                id_repeats = np.repeat(img_id, num_boxes_per_img)        # (K,)
                id_repeats = torch.from_numpy(id_repeats).to(device)
                ...
                box_with_img_id = torch.cat((id_repeats.reshape(-1,1),person_boxes),dim=1)    # (K,5)     # 5 = img_id,x1,y1,x2,y2
                ...
                # x2_out为：shape=(K1,256,16,16)
                x2_out = self.roi_heads[0](x2 , box_with_img_id)       
                # x1_out特征图为：shape=(K1,512,16,16)
                x1_out = self.roi_heads[1](x1 , box_with_img_id)       

                out_per_boxes = []          # [(768,16,16),(768,16,16),...]
                for k in range(x2_out.shape[0]):       # 第 k 个人
                    temp = torch.cat((x2_out[k],x1_out[k]),dim=0)       # shape=(256+512,16,16)
                    out_per_boxes.append(temp)

                # input_features: shape=(K1,768,16,16)       # K1 = 一个batch_size的人数
                input_features = torch.stack(out_per_boxes,dim=0)

                # (K,768,16,16) ---> pose_out: shape=(K1 , num_joints=17 , fsize=56 , fsize=56)
